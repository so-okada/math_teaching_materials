...
**Mathematical Background:**
With $u = g(x) = ax + b$ and $y = f(u) = u^c$, the chain rule gives
$$\frac{dy}{dx} = f'(u)\,g'(x) = c(ax+b)^{c-1} \cdot a.$$

For an actual step $h = \Delta x \neq 0$, the difference quotients obey the telescoping identity
$$\frac{\Delta y}{\Delta x} = \frac{\Delta y}{\Delta u} \cdot \frac{\Delta u}{\Delta x},$$
whose factors converge to $f'(u_0)$ and $g'(x_0)$ as $h \to 0$.

**Controls:**
- **Sliders $a$, $b$, $c$**: Stage parameters ($a \neq 0$), with live formulas
- **Point $x_0$**: Slider, or drag directly on the graphs
- **Step $h$**: Convergence table comparing difference quotients with the derivatives
...
### [Huygens' Cycloidal Pendulum](https://so-okada.github.io/math_teaching_materials/huygens_cycloidal_pendulum.html)
**File:** `huygens_cycloidal_pendulum.html`

Two ordinary pendulums and two cycloidal pendulums, one small-amplitude and one large-amplitude of each kind, start together. The ordinary pair slowly falls out of step; Huygens' pair stays in phase.

**Mathematical Background:**
An ordinary pendulum of length $L$ released from rest at angle $\theta_0$ has period
$$T(\theta_0) = 4\sqrt{\frac{L}{g}}\,K\!\left(\sin\frac{\theta_0}{2}\right), \qquad K(k) = \int_0^{\pi/2}\frac{d\varphi}{\sqrt{1-k^2\sin^2\varphi}},$$
which grows with the amplitude. If the bob is instead constrained to a cycloid generated by a circle of radius $a$ (string of length $L = 4a$ wrapping around two cycloidal cheeks), the arc length $s$ from the lowest point satisfies $\ddot s = -\dfrac{g}{4a}\,s$, so the period is
$$T = 2\pi\sqrt{\frac{4a}{g}} = 2\pi\sqrt{\frac{L}{g}},$$
independent of the amplitude (isochronism).

**Controls:**
- **Amplitude sliders**: Release angles of the two pendulums in each family
- **Start/Pause, Jump +60 s, Reset, Speed** (1×–100×)
- **Toggles**: Cycloidal cheeks, paths
- Side-by-side animations, time series with $T_0$ ticks, and a table of periods, elapsed cycles, and accumulated lag
...
- `huygens_cycloidal_pendulum.html`: Uses MathJax (CDN)
